...
Likelihood::Likelihood(cpp_model_func func)
 : f(func)
{ }
    ...
    vector<vector<double>> init, int init_iter)
{
    ...
    for (int i = init_iter; i < burn_in + iterations; ++i)
    {
        ...
        // Print progress
        if ((verbosity == 2 && i % 100 == 0) || (verbosity == 1 && i == burn_in + iterations))
        {
        ...
        {
            if (i < burn_in && i % (burn_in / 5) == 0)
            {
                double acceptance_rate = show_ar ? (double)count(acceptances.begin(), acceptances.end(), true) / ar_size : -1;
                cout << "[Burn-in; lp_0 = " << p[0] << "; acceptance rate = " << acceptance_rate << "]\n" << flush;
            }
            else if (i >= burn_in && (i - burn_in) % (iterations / 5) == 0)
            {
                double acceptance_rate = show_ar ? (double)count(acceptances.begin(), acceptances.end(), true) / ar_size : -1;
                cout << "[Sampling; lp_0 = " << p[0] << "; acceptance rate = " << acceptance_rate << "]\n" << flush;
            }
        }
